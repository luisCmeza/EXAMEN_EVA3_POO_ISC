...
public class Inventario {
    ...
    private int contador = 0;//Cantidad de objetos "reales" creados

    //AGREGA PRODUCTOS AL SISTEMA
    void addProduct(Producto producto) {
        if (contador < 10) {
            productos[contador] = producto;//Guarda en la posición cantidad el objeto producto creado en el main
            contador++;
        } else {
        ...
        for (int i = 0; i < productos.length; i++) {
            ...
            if (productos[i] != null && productos[i].getId() == productId) {
                ...
                for (int j = 1; j < contador; j++) {
                    if (productos[i] == null && i < contador - 1) {
